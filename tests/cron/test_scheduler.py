...
class TestDeliverResultWrapping:
    ...
    def test_delivery_wraps_by_default_when_wrap_argument_unset(self):
        """Pin: ordinary callers that don't pass `wrap` (i.e. every real cron
        delivery — the argument exists only for the synthetic operator-alert
        call) must see UNCHANGED behaviour: consult cron.wrap_response,
        defaulting True, same as before the `wrap` parameter existed. This
        guards against a future refactor silently flipping the default and
        stripping the header from every real user-facing delivery."""
        from gateway.config import Platform
        ...
        with patch("gateway.config.load_gateway_config", return_value=mock_cfg), \
            ...
            _deliver_result(job, "Here is today's summary.")  # wrap not passed
