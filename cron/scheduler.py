...
#   - repository-internal relative paths, 2 segments where the last has a
#     file extension: config/settings.yaml, hermes_cli/review_gate.py
# Two generic segments such as ``source/hash429abc.md`` are identifiers in
# script diagnostics as often as they are filesystem locations. Restrict that
# narrow case to known repository roots so we do not erase the useful source
# identifier while still withholding implementation paths.
_FS_PATH_RE = re.compile(
    r"(?:[A-Za-z]:\\[^\s'\"()]*"
    r"|(?:/[\w.\-]+){2,}/?"
    r"|[\w\-]+(?:/[\w\-]+){2,}"
    r"|(?:agent|cron|gateway|hermes_cli|job_intel|plugins|tests|tools|config)/[\w\-]+\.[A-Za-z0-9]{1,6}\b)"
)
        ...
        try:
            ...
            if success:
                ...
                try:
                    ...
                    operator_alert = None
                _failure_nudge = _failure_streak_nudge(job)
                if operator_alert is not None:
                    operator_alert += _failure_nudge
                else:
                    deliver_content = (deliver_content or "") + _failure_nudge
                if blocked_config:
                    # Bypass the generic failure summarizer (whose auth/timeout
                    # heuristics would mislabel this as a provider runtime
                    # failure) — say plainly that config validation blocked the
                    # run and nothing was spent. The audience decision above
                    # still stands: whichever target was going to receive this
                    # failure receives this text instead, so a config problem
                    # on an end-user job still reaches the operator only.
                    _pf_text = re.sub(
                    ...
                    if operator_alert is not None:
                        operator_alert = _blocked_text
                    else:
                        deliver_content = _blocked_text
                if drift_skip:
                    # Preserve the upstream drift-guard remediation in full while
                    # routing it through the local audience-policy decision.
                    _drift_text = re.sub(
                    ...
                    if operator_alert is not None:
                        operator_alert = _drift_delivery
                    else:
                        deliver_content = _drift_delivery
