    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                #
                # A turn whose only tool call is `clarify` is internal
                # bookkeeping: the clarifying question reaches the user through
                # the clarify handler itself, and emitting the assistant row as
                # interim commentary duplicates it as a status bubble.
                if (
