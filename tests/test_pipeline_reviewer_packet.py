...
def test_reviewer_packet_includes_tracked_diff() -> None:
    module = importlib.import_module("hermes_cli.pipeline_reviewer_packet")
    ...
    packet = module.build_reviewer_packet(
        ...
        engineer_output=_engineer_output(),
        baseline_snapshot=_snapshot(head_sha="abc123"),
        post_snapshot=_snapshot(head_sha="abc123", unstaged_files=("tracked.txt",)),
        git_result=_git_result(
    ...
    assert git_section["tracked_diff"] == sample_diff


def test_reviewer_packet_tracked_diff_unavailable_when_none() -> None:
    module = importlib.import_module("hermes_cli.pipeline_reviewer_packet")

    packet = module.build_reviewer_packet(
        pipeline_id="engineering_review_pipeline",
        task_summary="No diff supplied",
        engineer_output=_engineer_output(),
        baseline_snapshot=_snapshot(head_sha="abc123"),
        post_snapshot=_snapshot(head_sha="abc123", unstaged_files=("tracked.txt",)),
        git_result=_git_result(
    ...
    assert git_section["tracked_diff_available"] is False
    assert git_section["tracked_diff"] == ""
