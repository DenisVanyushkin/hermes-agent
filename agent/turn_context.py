...
def invoke_pre_llm_call_hook(
    ...
    *,
    ...
    plugin_user_context = ""
    try:
        # lifecycle, not plugins: it notifies first-party observers first and
        # then delegates to plugins.invoke_hook, so a turn fired through this
        # helper is visible to observability exactly like every other hook.
