...
- [x] Инварианты соблюдены во всех разделах
- [x] Ни один пункт не правит `provider_payload` так, чтобы в него попадали
...
- [x] Transport receipt и V2-envelope остаются разными артефактами; authority-поля
      не дописываются в generic transport receipt.
- [x] Каждая правка сопровождается тестом, который **падает до** правки и
      **проходит после**; «тест написан» без зафиксированного красного прогона
      не засчитывается.
- [x] Тесты запускаются интерпретатором worktree, а не gateway-venv: под
      gateway-venv часть проверок ложно краснеет из-за pysqlite3-шима
      ([#6](https://github.com/DenisVanyushkin/hermes-agent/issues/6)).
- [x] Ограничение по времени задаётся внешним `/usr/bin/timeout`, а не аргументом
      `--timeout`: плагина `pytest-timeout` в каноническом venv нет.
- [x] Полный `pytest` по репозиторию на VPS не запускается: он даёт loadavg ~50
      и systemd-вотчдог убивает живой гейтвей. Только адресные наборы.
- [x] Прогон без итоговой строки pytest считается убитым, а не «чистым».
...
> Измерение на VPS hermes-agent (2026-08-25): сборка артефакта — `114.001 s`,
> установка — `10.100 s`, supervised target — `0.997 s`, полный smoke до
> именованного blocker — `158.22 s`. Таймауты runner: build `300 s`, target
> `30 s`, evaluation `60 s`.

- [x] Измерить фактическое время сборки артефакта на VPS и записать число в план
      или в коммит; из #8 известно, что оно «заметно больше 90 s» при архиве по
      ~11 000 файлов, но точная цифра не зафиксирована.
- [x] Поднять таймауты `subprocess.run` в `scripts/gate_b_composition_smoke.py`
      исходя из измеренного времени с явным запасом.
- [x] Сделать так, чтобы отсутствие обязательной предпосылки печаталось как
      именованная ошибка предпосылки, а не как обрыв по таймауту.

**DoD**

- [x] Измеренное время сборки записано числом, а не оценкой.
- [x] Прогон с намеренно удалённой предпосылкой (например, удалённым spend
      record) завершается быстро и печатает, **какая именно** предпосылка не
      выполнена.
- [x] Прогон в штатных условиях не упирается в таймаут.

### 3.3 Smoke реально ходит через production-адаптер

- [x] **Пункт 3.3 выполнен**
...
- [x] Довести прогон до вызова `decision_request_factory` и доказать вызов
      наблюдаемым следом (лог, счётчик, артефакт), а не тем, что имя стоит в
      конфиге.
- [x] Убедиться, что фабрикой оказывается `build_decision_request_from_context_v2`,
      а не тестовая фикстура.

**DoD**

- [x] В выводе прогона присутствует наблюдаемое доказательство вызова
      production-фабрики.
- [x] Подмена фабрики на заведомо ломающуюся ломает прогон — то есть проверка
      действительно зависит от неё, а не проходит мимо.

### 3.4 Smoke читает corpus authority, а не хранит SHA в коде

- [x] **Пункт 3.4 выполнен**
...
- [x] Убрать `CORPUS_SHA256` как захардкоженную константу из
      `tests/product_search/gate_b_cli_smoke_fixture.py:37`.
- [x] Заставить фикстуру читать corpus authority из машиночитаемого источника,
      путь к которому настраивается, и **проверять** её, а не принимать на веру.
- [x] Добавить тест на fail-closed: при расхождении прочитанной authority с тем,
      что потребляет боевой путь, smoke падает с именованной ошибкой.
- [x] Тест использует временную тестовую authority, а не производственное значение.

**DoD**

- [x] `grep -rn "b1db802dbb3d" tests/product_search/gate_b_cli_smoke_fixture.py`
      не даёт совпадений.
- [x] Подмена authority-файла в тесте меняет то, что читает smoke, без правки его
      кода.
- [x] Тест на fail-closed падает, если проверку расхождения убрать.
...
- [x] **Order 1 закрыт**
- [x] Раздел 1 отмечен выполненным.
- [x] Раздел 2 отмечен выполненным, включая **DoD пункта 2.8** — он является
      нормативным полным критерием сквозного E2E; здесь он не переформулируется,
      чтобы две версии не разъехались.
- [x] Раздел 3 отмечен выполненным.
- [x] Композиционный smoke на собранном артефакте доходит до конца и печатает
      результат.
- [x] Ни одна отметка `[x]` не поставлена без указания теста, команды или файла,
