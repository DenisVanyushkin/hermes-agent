...
def _headless_session_is_cron(session_key: str) -> bool:
    ...
    ``_is_cron_approval_context()`` reads the per-session context first and
    only falls back to the process-global env flag, which is racy in the
    shared gateway process (pool threads can carry stale interactive
    contextvars). The ``cron_<job>_<ts>`` session-key shape is accepted as an
    equally valid signal for the same reason.
    """
    return _is_cron_approval_context() or str(session_key or "").startswith("cron_")
...
def check_all_command_guards(command: str, env_type: str,
    ...
    cron_mode = _get_cron_approval_mode() if _is_cron_approval_context() else ""
