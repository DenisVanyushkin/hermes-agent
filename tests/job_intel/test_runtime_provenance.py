...
def test_capture_runtime_provenance_includes_expected_topology(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(runtime, "_module_origin", lambda name: f"{workdir}/{name.replace('.', '/')}.py" if name in {"job_intel.runtime", "job_intel.store", "job_intel.browser_sourcing", "job_intel.cli", "requests"} else None)
    ...
    assert provenance["effective_workdir"] == str(workdir)
    ...
    assert provenance["imported_module_locations"]["job_intel.runtime"] == f"{workdir}/job_intel/runtime.py"
    assert provenance["imported_module_locations"]["requests"] == f"{workdir}/requests.py"
