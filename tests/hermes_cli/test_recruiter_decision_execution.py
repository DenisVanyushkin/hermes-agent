...
class TestControllerRecruiterBundleDispatch:
    """Task 4: controller resolves helper by recruiter routing bundle."""

    def test_recruiter_pipeline_dispatches_application_helper_for_materials_prompt(self) -> None:
        prompt = "подготовь резюме и сопроводительное письмо для вакансии"
        session = SimpleNamespace(
            pipeline_id=RECRUITER_PIPELINE_ID,
            pipeline_session_id="ps-1",
            router_status="selected",
