...
def format_arm_refused_alert(path) -> str:
    """Сторож не включился на старте: бюджет не пишется."""
    return "\n".join(
        [
            "🔴 Гермес: сторож устаревшего кода НЕ включился",
            f"На старте не завёлся бюджет рестартов: {path} не пишется.",
            "Авторестарт выключен — уехавший от диска процесс сам не перезапустится.",
            "Дальше руками: проверь права на HERMES_HOME, потом hermes gateway restart",
        ]
    )


def format_budget_write_failed_alert(path) -> str:
    """Рестарт уже запланирован, а метка бюджета не легла — сторож выключен."""
    return "\n".join(
        [
            "🔴 Гермес: авторестарт отключён",
            f"Рестарт запланирован, но метку бюджета не записал: {path}.",
            "Сторож выключен до перезапуска процесса — устаревший процесс "
            "автоматически рестартовать больше не буду.",
            "Дальше руками: hermes gateway restart",
        ]
    )
