...
@pytest.mark.asyncio
async def test_generate_summary_async_empty_choices_falls_back_cleanly():
    """Empty async summarizer responses should not IndexError and must fall back."""
    from trajectory_compressor import CompressionConfig, TrajectoryCompressor, TrajectoryMetrics
    ...
    compressor._use_call_llm = False
    async_client = MagicMock()
    async_client.chat.completions.create = MagicMock(return_value=SimpleNamespace(choices=[]))
    compressor._get_async_client = MagicMock(return_value=async_client)
