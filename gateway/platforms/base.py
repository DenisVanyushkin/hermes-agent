...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    if not candidate:
        return None
    for cand in _host_visible_media_candidates(candidate):
        # translate_container_paths=False: the candidate list above is already
        # the complete, deliberately narrow set of host paths this container
        # path may resolve to.
        resolved = _validate_host_media_path(cand, translate_container_paths=False)
        if resolved:
            return resolved
    return None


def _validate_host_media_path(path: str, *, translate_container_paths: bool = True) -> Optional[str]:
    ...
    #
    # Callers that already computed their own host candidates pass
    # translate_container_paths=False: this resolver maps ANY configured
    # container mount, which is deliberately wider than the narrow
    # cache-dirs + /output export policy in _host_visible_media_candidates.
    # Running both would silently widen that policy back out — every rw bind
    # mount (a source checkout, a service dir) would become a delivery
    # channel again.
    translated = (
        _translate_docker_container_media_path(expanded)
        if translate_container_paths
        else None
    )
