...
REVIEW_INSTRUCTION = (
    ...
    "в raw_text).\n\n"
    "ГРАНИЦА КОРПУСА: сообщения ниже — образцы СТИЛЯ, а не источник истины "
    "о жизни Амины. Не выводи из них события, даты, планы и договорённости "
    "и не сопоставляй сообщения разных дней между собой: данные могли "
    "измениться или быть удалены уже после отправки, а корпус об этом не "
    "знает. Содержательная правильность сообщения — ВНЕ твоей оценки; "
    "правдивость проверяется ТОЛЬКО сопоставлением final с raw_text ОДНОГО "
    "И ТОГО ЖЕ сообщения. Если тянет написать «здесь пропущено событие» или "
    "«на самом деле в этот день было X» — это как раз тот вывод, которого "
    "делать нельзя.\n\n"
