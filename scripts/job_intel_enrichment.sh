...
resolve_db_path() {
  ...
  # dir.  $HOME/.hermes/job_intel/job_intel.sqlite3 is a stale copy frozen in
  # May 2026 and must never be targeted -- enriching it is a silent no-op.
...
export JOB_INTEL_STATE_DIR="${JOB_INTEL_STATE_DIR:-/var/lib/job-intel/state}"
...
exec "$python_bin" -m job_intel enrichment
