...
class OpsOperation:
    ...
    # Неизменная приставка argv без sudo: по ней ops_capability_hint отвечает на
    # вопрос «покрыта ли эта команда каталогом». Поле обязательное -- забытая
    # сигнатура сделала бы операцию невидимой для подсказки, а тишина здесь
    # неотличима от честного «такой операции нет».
    signature: tuple[str, ...]
...
CATALOG: dict[str, OpsOperation] = {
    ...
    for op in (
        ...
        OpsOperation("gateway_restart", RISK_MUTATE, _gateway_restart_argv,
                     ...
                     signature=("hermes", "gateway", "restart")),
        OpsOperation("git_push_force_with_lease", RISK_DESTROY, _git_push_force_with_lease_argv,
                     ...
                     signature=("git", "push", "--force-with-lease"),
