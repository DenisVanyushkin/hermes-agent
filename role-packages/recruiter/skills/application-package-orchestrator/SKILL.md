...
7. **Deliver** in this order (full-package format; collapse for subsets):
   ...
   **File format — Word by default:** materials (CV, cover letter, recruiter message, questionnaire answers) are delivered as real `.docx` files produced with python-docx (`pip install python-docx` in the sandbox if not yet installed — it installs in seconds and persists in the container). Use proper document structure: headings, bullet lists, no markdown syntax leaking into text. `package_qa_report.md` stays Markdown. Deliver Markdown materials ONLY if the user explicitly asks for Markdown, or if python-docx installation fails — in the latter case say so explicitly.
