...
- [ ] Написать прямой воспроизводитель: настоящий `RecordingStore` под
...
- [ ] Добавить контрольную группу в том же тесте: та же запись через
      `RecordingStore` напрямую читается успешно — чтобы тест доказывал, что
      разница именно в декораторе, а не в записи.
- [ ] Зафиксировать отсутствие боевой достижимости:
      `grep -rn "_AuthorityRecordingStore(" job_intel/ tests/ scripts/` даёт пусто.
- [ ] Записать в план или в коммит, что evidence issue #9 относится к состоянию до
      `802dd48698`, и какой именно факт устарел.

**DoD**

- [ ] Воспроизводитель **падает** на базовом HEAD с `recording_corrupt` (или
      эквивалентным `LLMProviderError`), вывод прогона с итоговой строкой приложен.
- [ ] Ветка контрольной группы (`RecordingStore` напрямую) в том же прогоне
      **проходит** — иначе тест меряет не декоратор.
- [ ] Отсутствие инстанцирований зафиксировано выводом команды, а не утверждением.
...
- [ ] Удалить мёртвый `_AuthorityRecordingStore`, а не возвращать его в боевой путь:
...
- [ ] Закрепить текущую архитектуру тестом: generic transport record остаётся raw,
      round-trip через `SemanticRecordingStore` проходит, authority-полей в нём нет.
- [ ] Закрепить вторым тестом, что `v2_record` содержит все пять authority-полей и
      `semantic_transport_record_sha256` **до** вызова `seal_record`.
- [ ] Если исполнитель найдёт живого потребителя декоратора и решит его сохранить —
...
- [ ] Оба закрепляющих теста проходят; тест на порядок сборки `v2_record` падает,
      если authority-поле перенести после `seal_record` (проверяется мутацией).
- [ ] После удаления декоратора адресный набор тестов product_search и semantic
      runtime зелёный, вывод с итоговой строкой приложен.
- [ ] Команда `grep -n "seal_record"` по обоим модулям используется как подсказка
...
- [ ] Мутационный тест: подмена каждого из пяти authority-полей V2-envelope после
      сохранения детектируется как нарушение печати.
- [ ] Контрольная немодифицированная запись проходит тот же verifier.
- [ ] Найти всех потребителей generic semantic recording
...
- [ ] Мутационный тест проходит по каждому authority-полю по отдельности.
- [ ] Адресный прогон тестов semantic recording зелёный, вывод с итоговой строкой
      приложен.
- [ ] Все четыре критерия приёмки #9 отмечены по одному, с указанием теста,
