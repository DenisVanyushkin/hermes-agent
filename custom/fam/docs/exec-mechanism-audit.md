...
## Follow-up (НЕ делать здесь, отдельная задача)
1. Поправить `SKILL.md:86-88` — убрать неверное "trip the dangerous-command
   approval gate" для plain chaining; переформулировать как operational
   hygiene (предсказуемость/аудируемость команд), а не security control.
2. (Опционально, отдельная security-задача) либо добавить в
