...
def test_one_step_adapter_preserves_machine_captured_pytest_payload() -> None:
    module = importlib.import_module("hermes_cli.pipeline_one_step_execution")
    adapted = module._adapt_runner_result(
        invocation_result=SimpleNamespace(
            ok=True,
    ...
    assert safe["tool_call_summaries"][0]["tool_name"] == "pytest"
    assert safe["tool_call_summaries"][0]["status"] == "succeeded"
    assert safe["tool_call_summaries"][0]["result_payload"]["status"] == "passed"
