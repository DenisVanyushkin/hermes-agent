...
## Phase — meds defer (заморозка дозы до времени) (2026-07-24)

Третья реакция на активное напоминание о лекарстве, помимо `taken`/`skip`:
**отложить одну дозу на позже** (кейс: 09:00 «пора принять мисол» →
«напомни в 20:00, раньше не могу»).

- CLI: `fam med defer <intake_id> --until HH:MM` (HH:MM = сегодня Алматы;
  либо ISO). intake_id — из `fam med list --pending`. Двигает
  `series_next_utc` дозы на целевое время (доза остаётся `pending`);
  персистентная серия сама возобновляется в 20:00. exit 2 на
  прошлом/после-полуночи/битом времени. audit `meds.defer`.
- Три реакции на дозу: `fam med taken` (принято, остаток−1) /
  `fam med skip` (отменить эту дозу) / `fam med defer` (позже, доза жива).
- **pending-acks.json знает про deferral** (schema 11, новая колонка
  `med_intakes.deferred_until_utc`): отложенная доза в снимке показывается
  как «запланирован на 20:00», а не «пропущено» — свежая/сброшенная сессия
  не пере-напоминает раньше времени. `cmd_med_defer` обновляет снимок
  сразу (`acks.write`), как taken/skip.
- Скилл amina-fam v17: правило различения трёх реакций + предупреждение
  «после 21:30 напоминания молчат до утра — точно на HH:MM?».
- Таймеры/тик НЕ менялись (серия уже умела `series_next_utc`). Прод
  мигрирован schema 11 (бэкап `assistant.db.bak-schema11-20260724`).
- Известные minor (финальное ревью): defer дозы с remaining==0 работает
  как обычно (осознанно); поздний defer >12ч от plan выпадает из
  acks-снимка после plan+12ч (тик всё равно нагнетает; реальный
  09:00→20:00 не затронут).
