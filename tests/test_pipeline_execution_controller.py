...
def test_autonomous_executor_bridge_uses_subagent_runner_and_observed_git_delta(tmp_path: Path) -> None:
    ...
    assert "✅ ЗАДАЧА ВЫПОЛНЕНА" in result.final_response_text
    assert "- engineer_notes.txt" in result.final_response_text
    assert "Ничего не закоммичено. Ответь «коммить» — закоммичу изменения, «отмена» — сброшу." in result.final_response_text
