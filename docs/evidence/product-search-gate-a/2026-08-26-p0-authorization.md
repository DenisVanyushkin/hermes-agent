...
Путей в таблице: 24. Проверяется скриптом по объединению путей
...
| `tests/acceptance/test_acquisition_gate.py` | B1 | поправка 2026-08-27: переименование ключа улики |
| `tests/deploy/test_product_search_experiment_runner.py` | A4 | поправка 2026-08-27: рукописная фикстура манифеста отстала от A4 |
...
**Поправка 2026-08-27: два тестовых файла.** Пункт B1 требует переименовать
`cell_states` в `acquisition_outcomes`, и валидатор улики обязан следовать за
переименованием, иначе боевой прогон не пройдёт собственную проверку.
`tests/acceptance/test_acquisition_gate.py` подаёт валидатору старый ключ и от
этого ломается. Обход через приём обоих ключей отвергнут сознательно: старое
имя, выжившее в валидаторе, — ровно та ложь, против которой написан пункт.

`tests/deploy/test_product_search_experiment_runner.py` сломался раньше и по
другой причине: срез A4 (`1659b56e00`) ввёл обязательные поля версии и хеша
каталога причин исключения в манифесте. Боевой строитель `build_experiment_manifest`
их отдаёт, а рукописная фикстура `valid_manifest` в этом тесте — нет, поэтому
три проверки падают на новом требовании, не дойдя до того, что они проверяют.
Дефекта в боевом коде здесь нет; отстала фикстура. Регрессия установлена
бисекцией: зелено на `c5108a2098`, красно на `1659b56e00`.

Правка ограничена приведением обоих файлов в соответствие с уже
санкционированными изменениями: в acceptance — переименование ключа и
утверждения о нём, в deploy — дополнение фикстуры манифеста обязательными
полями. Разрешение на изменение предмета этих тестов поправкой не выдаётся.
Число путей в таблице: 22 → 24.

**Поправка 2026-08-28: часовой пояс браузера.** Владелец решил, что Gate A
работает с авторизованным LinkedIn, и потребовал не потерять авторизацию,
проводя вход через алматинский выход. Выход проверен изнутри самого браузера:
`95.56.129.108`, Kazakhtelecom, Алматы. Но браузер сообщает часовой пояс
`Europe/Berlin`, унаследованный от хоста в Contabo, то есть казахский
резидентный адрес приходит с европейским временем — сочетание, по которому
LinkedIn распознаёт VPN.

Риск не гипотетический и уже реализовывался на этом хосте в другом виде:
токены codex отзывались провайдером при расхождении страны входа и страны
сервера (`codex-auth-geo-invalidation`).

Часовой пояс задаётся окружением, в котором стартует Chromium, то есть блоком
`start_as_browser` в `scripts/browser-desktop-bootstrap.sh`. Прежняя поправка
к этому файлу от 2026-08-26 разрешала **только** превращение выбора network
namespace в параметр, поэтому изменение окружения требует отдельного
разрешения, которое настоящей поправкой и выдаётся.

Правка ограничена добавлением переменной `TZ` в окружение браузера, со
значением по умолчанию, не меняющим поведение профилей вне namespace:
поведение `hh` и прочих профилей остаётся прежним и проверяется тестом.
Разрешение на изменение прочих свойств скрипта этой поправкой не выдаётся.
Число путей в таблице не меняется: файл в ней уже есть.
