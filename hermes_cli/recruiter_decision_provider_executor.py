...
_MODULE_CONTRACTS: dict[str, str] = {
    "company_research": (
        ...
        "layoff_tracker/hiring_signal), date_or_access_timestamp (ISO date; use dates mentioned in a "
        "snippet when present, otherwise the provided access_date), confidence (low/medium/high), "
        "fact_vs_inference (fact/recent_public_signal/inference/unknown). Two allowed evidence pools: "
        "(1) the provided posting/context — source_type company_website for the about-company section, "
        "hiring_signal for the vacancy itself; (2) the provided web_search_results — source MUST be "
        "that result's URL, source_type news/funding_announcement/layoff_tracker/employee_reviews as "
        "appropriate, fact_vs_inference recent_public_signal for news snippets. Extract ONLY what the "
        "posting or a search snippet actually states. Never invent numbers, never merge snippets into "
        "conclusions beyond what they say; omit what is not stated."
    ),
    "vacancy_assessment": (
