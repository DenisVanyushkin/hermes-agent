...
def test_unresolved_cross_thread_engineering_task_uses_bounded_agent_resolution(
    monkeypatch,
):
    _CapturingAgent.last_init = None
    _CapturingAgent.run_calls = 0
    ...
    assert _CapturingAgent.run_calls == 1
    assert result["final_response"] == "ok-from-agent"
    assert "cronjob" not in _CapturingAgent.last_init["enabled_toolsets"]
    assert "kanban" not in _CapturingAgent.last_init["enabled_toolsets"]
    assert "code_execution" not in _CapturingAgent.last_init["enabled_toolsets"]
    assert set(_CapturingAgent.last_init["enabled_toolsets"]) == {
        "clarify",
        "delegation",
        "session_search",
    }
...
def test_cross_thread_resolution_prefetches_authenticated_slack_context(monkeypatch):
    ...
    assert "FETCHED PLAN SENTINEL" in _CapturingAgent.last_user_message
    assert "FETCHED PLAN SENTINEL" not in _CapturingAgent.last_init[
...
def test_cross_thread_resolution_fails_closed_in_proxy_mode(monkeypatch):
    ...
    assert result["api_calls"] == 0
    assert result["tools"] == []
    assert "proxy" in result["final_response"].lower()
